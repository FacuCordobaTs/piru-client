...
export function OrderSummaryItemDetails({ item, textColor }: { item: any, textColor?: string }) {
    ...
    return (
        <div className="min-w-0">
            ...
            {agregados.map((ag, idx) => (
                <p key={ag.id ?? `ag-${idx}`} className={`text-xs ${textColor || 'text-primary/85'} font-medium mt-0.5`}>
                    Con: {ag.nombre}
